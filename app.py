...
# --- Crear Excel de ejemplo con 12 productos ---
...
# --- Crear imagenes de ejemplo para los 12 productos ---
    ...
    def draw_logo(self):
        for ext in ['png', 'jpg', 'jpeg']:
            ...
            if os.path.exists(path):
                self.image(path, x=245, y=193, w=30, type=ext)  # Logo más abajo  # Mover logo a la esquina inferior derecha
                break

def generar_pdf_estilo_original(datos, salida="catalogo_estilo_original.pdf"):
    pdf = OfertaPDF()
    pdf.draw_title()
    start_x = 25  # Centrado horizontal mejorado
    start_y = 20  # Aún más arriba para centrar las 3 filas
    cols = 4
    spacing_x = 65
    spacing_y = 70  # Reducimos para no bajar tanto la última fila
    ...
    for i, row in datos.iterrows():
        ...
        if row_pos == 0:
            y += 5  # bajar 5 mm la primera fila
        elif row_pos == 1:
            y -= 10  # subir 5 mm la segunda fila
        elif row_pos == 2:
            y -= 20  # subir 2 cm la tercera fila
